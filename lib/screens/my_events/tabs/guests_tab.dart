...
class _GuestsTabState extends State<GuestsTab> {
  ...
  Widget build(BuildContext context) {
    // ✅ Fix: Get expected guests from the selected event
    final event = Provider.of<UserEventProvider>(context, listen: false).selectedEvent;

    // Safely parse guestCount (assuming it's a string like "500" or an int)
    int expectedGuests = 100; // Default fallback
    if (event != null) {
      if (event.guestCount is int) {
        expectedGuests = event.guestCount as int;
      } else {
        ...
        builder: (context, provider, child) {
          ...
          return CustomScrollView(
            slivers: [
                    ...
                    child: Center(
                      child: Column(
                        children: [
                          ...
                          Text(
                            "No VIPs added yet.\nAdd special guests manually to issue direct passes.",
              ...
              // ─── 5. REGULAR GUESTS HEADER (Show who RSVP'd online) ───
