...
class PaymentProvider with ChangeNotifier {
  ...
  Future<bool> processPayment({
    ...
    try {
      ...
      String txnId = paymentIntent['id'] ?? "STRIPE_${DateTime.now().millisecondsSinceEpoch}";
  ...
  Future<bool> refundPayment(String paymentId) async {
    ...
    try {
      ...
      final eventId = paymentData['eventId']?.toString();

      WriteBatch batch = _firestore.batch();
      batch.update(paymentDoc.reference, {'status': 'refunded', 'refundedAt': FieldValue.serverTimestamp()});
  ...
  Future<String> exportPaymentHistory(String userId) async {
    try {
      QuerySnapshot snapshot = await _firestore
          .collection('payments')
          .where('userId', isEqualTo: userId)
      ...
      csv.writeln('Date,Transaction ID,Event,Amount,Method,Status');

      for (var doc in snapshot.docs) {
        final data = doc.data() as Map<String, dynamic>;
        final timestamp = (data['timestamp'] as Timestamp?)?.toDate();
        final dateStr = timestamp != null ? '${timestamp.day}/${timestamp.month}/${timestamp.year}' : 'N/A';

        csv.writeln('$dateStr,${data['transactionId'] ?? 'N/A'},${data['eventName'] ?? 'N/A'},${data['amount'] ?? 0},${data['method'] ?? 'N/A'},${data['status'] ?? 'N/A'}');
      }
      return csv.toString();
    } catch (e) {
      return '';
